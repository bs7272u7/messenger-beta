(function () {
    ...
    let game = null, selected = null, pendingPromotion = null, flipped = false, lastMove = null, replayFen = null;
    ...
    function renderMeta() {
        ...
        const status = game.status === "waiting" ? "친구가 초대 코드로 입장하기를 기다리는 중입니다." : game.status === "active" ? `${game.turn === "w" ? "백" : "흑"} 차례${game.check ? " · 체크" : ""}` : "게임 종료";
    ...
    function applyState(state) { game = state; replayFen = null; renderBoard(); renderMeta(); }
